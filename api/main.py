...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # En producción: tu dominio específico
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def upload_lista(
    ...
    tipo: Optional[str] = Form(None),  # DH | CAJAS | LISTA_E | None=autodetectar
...
@app.get("/api/listas")
async def get_listas(proveedor_id: Optional[int] = None):
    where = "WHERE 1=1"
    params = []
    if proveedor_id:
...
async def desactivar_lista(lista_id: int):
    async with app.state.pool.acquire() as conn:
